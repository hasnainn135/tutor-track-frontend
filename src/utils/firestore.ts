...
export const getMyTutors = async (
    ...
    try {
        if (studentData.myTutors) {
            const fetchTutorsPromises = studentData.myTutors.map((tutor) => getDoc(doc(db, "users", tutor.tutorId)))
...
export const getMyStudents = async (
    ...
    try {
        if (tutorData.myStudents) {
            const fetchStudentsPromises = tutorData.myStudents.map((student) => getDoc(doc(db, "users", student.studentId)))
