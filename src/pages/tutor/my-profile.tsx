...
const MyProfile = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      if (!userData) return;
      try {
        const reviews = await getAllReviews(userData.uid);
  ...
  if (
    ...
    !tutorData.displayChargesPerHour ||
    ...
    return (
      <ContainerLayout>
        <p>
          Please enter data about yourself in{" "}
          <Link href="/tutor/settings" className="underline">
            Settings
          </Link>{" "}
